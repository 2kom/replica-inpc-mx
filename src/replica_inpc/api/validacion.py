...
def validar_mensual(
    ...
    Requiere periodos mensuales (PeriodoMensual). Detecta tipo desde el
    resultado. Ver docs/diseño.md §6.2.
    """
    periodos = resultado.df.index.get_level_values("periodo")
    if not isinstance(periodos[0], PeriodoMensual):
        raise ErrorConfiguracion(
            "validar_mensual requiere un ResultadoCalculo mensual. "
            "Si partes de un resultado quincenal, aplica a_mensual() antes."
        )
...
def validar_quincenal(
    ...
    Requiere periodos quincenales (PeriodoQuincenal). Ver docs/diseño.md §6.2.
    """
    periodos = resultado.df.index.get_level_values("periodo")
    if not isinstance(periodos[0], PeriodoQuincenal):
        raise ErrorConfiguracion(
