...
- [Diseño del sistema — replica-inpc-mx](#diseño-del-sistema--replica-inpc-mx)
  ...
  - [11. Decisiones de diseño](#11-decisiones-de-diseño)
    ...
    - [11.14 UTF-8 como primer encoding en `LectorSeriesCsv`](#1114-utf-8-como-primer-encoding-en-lectorseriescsv)
    - [11.15 Dispatch interno en `CalculadorBase`](#1115-dispatch-interno-en-calculadorbase)
    - [11.16 Vectorización del loop interno de `validacion/indices.py`](#1116-vectorización-del-loop-interno-de-validacionindicespy)
    - [11.17 `LaspeyresEncadenado` — derivación de `f_h`](#1117-laspeyresencadenado--derivación-de-f_h)
    ...
    - [11.18 Imputación de faltantes en series](#1118-imputación-de-faltantes-en-series)
    - [11.19 `empalmar` — combinación histórica](#1119-empalmar--combinación-histórica)
    - [11.20 `RENOMBRES_INDICES` y normalización cross-versión](#1120-renombres_indices-y-normalización-cross-versión)
    - [11.21 `empalmar` — topología PATH](#1121-empalmar--topología-path)
...
Las versiones encadenadas normalizan cada índice por `f_k` (columna `encadenamiento` de la canasta) y aplican un `factor_h` de empalme al resultado. Las fórmulas exactas y la derivación de `f_k` están en §5.6 y §11.18.
...
**Columna `indice_incidencia`.** Junto a `indice_replicado`, cada calculador puebla la columna interna `indice_incidencia` (no expuesta en ninguna vista pública; ver [5.7](#57-resultadoindice)) en la escala compatible con Laspeyres que usan las incidencias ([11.29](#1129-indice_incidencia-y-de-encadenamiento-de-incidencias)): los encadenados (T1/T2) guardan `i_tramo` — el nivel **antes** de multiplicar por `factor_h`; los directos guardan el nivel crudo, que coincide con `indice_replicado` salvo cuando el directo actúa como T0 de un encadenado, donde también es el nivel antes de `factor_h`. Periodos `sin_datos`/`fallida` → `NaN`.
...
La columna interna `indice_incidencia` ([5.7](#57-resultadoindice)) se promedia con las **mismas** máscaras que `indice_replicado`. Como `a_mensual` reconstruye el `df_result`, debe agregarla de forma explícita — a diferencia de `empalmar`/`rebasar`, que la arrastran/preservan sin tocarla (el rebase NO la reescala, así la incidencia queda invariante al rebase; ver [11.29](#1129-indice_incidencia-y-de-encadenamiento-de-incidencias)).
...
- `version_t != version_base` (cross-canasta) → `i_tramo` directo es escala interna discontinua en la junta (cruzarla daría incidencias de ~−29 pp). Para tipos con **clasificación estable** la fila se calcula **exacta por segmentos** (Fase 2A): `Σ_m f_INPC^(m) · w_K · ΔJ_K / INPC_visible(b)`, marcada `cross_segmentado`/`cross_sin_frontera` en `metodo_incidencia`. Aplica a las 3 juntas, T1 incluida: el ancla del lado nuevo se deriva por continuidad del visible, no se supone igual a 100. Para tipos finos sin clasificación estable cae al `indice_replicado` visible sin garantía (`cross_visible`, diferido a Fase 2B). El método por fila se publica en `.reporte`/`.diagnostico` (`metodo_incidencia`), no en `.resultado.largo`; el cruce es además detectable por `version_t != version_lag`. Detalle en [11.29](#1129-indice_incidencia-y-de-encadenamiento-de-incidencias).
...
Las funciones de serie consumen la columna interna `indice_incidencia` (de-encadenada, vía `ResultadoIndice._completo`) y eligen la escala por fila `(periodo, indice)`: within-canasta usa `indice_incidencia` (exacto); cross-canasta de tipos con clasificación estable usa la descomposición exacta por segmentos (Fase 2A), y de tipos finos cae al índice visible (sin garantía, `cross_visible`). El método por fila se marca en `metodo_incidencia` (`.reporte`/`.diagnostico`); el cruce es además detectable por `version_t != version_lag`. No se agrega ninguna columna nueva a `.resultado`. Detalle del contrato en el Fix 2 de [5.11](#511-cálculo-de-variaciones-e-incidencias) y la decisión en [11.29](#1129-indice_incidencia-y-de-encadenamiento-de-incidencias).
...
| `rellenado` | 1 | Algún genérico del periodo fue imputado por bfill/ffill (ver §11.23) |
...
### 11.8 Detección de `null_por_faltantes`
...
### 11.11 Schema condicional en `ReporteDetalladoValidacion`
...
### 11.12 `INDICES_VALIDABLES` en el dominio
...
### 11.13 Cache de clase en `FuenteValidacionApi`
...
### 11.14 UTF-8 como primer encoding en `LectorSeriesCsv`
...
### 11.16 Vectorización del loop interno de `validacion/indices.py`
...
### 11.17 `LaspeyresEncadenado` — derivación de `f_h`
...
### 11.18 Imputación de faltantes en series
...
### 11.19 `empalmar` — combinación histórica
...
### 11.22 `rebasar` — huérfanos con `UserWarning`
...
### 11.23 `bfill→ffill` y estado `"rellenado"`
...
### 11.24 Autoreload IPython — `type(self)._PROXY`
...
### 11.26 Re-export de errores y tipos en `replica_inpc/__init__.py`
...
### 11.27 `a_mensual` — filtrado de manifiestos huérfanos
...
**Fase 2A — cross-canasta exacto por segmentos (implementado).** Para todo tipo con clasificación estable (mismo conjunto de categorías y ningún genérico cruza de bucket en las 3 juntas) la fila cross se calcula exacta:
...
Los insumos (`J_K_old(e)`, `I_K_visible(e)`, `INPC_visible(e)`) en **quincenal** están en `_df_resultado`; en **mensual** `a_mensual` promedia la quincena de enlace, así que se preservan en el campo interno `_frontera` ([5.7](#57-resultadoindice), [5.10](#510-conversión-y-combinación)). Componentes: `_partir_en_segmentos`, `_calcular_incidencia_cross_encadenada`, `_construir_lector_ancla`, `_verificar_operandos`, `_es_clasificacion_estable` (`incidencias.py`); `_construir_frontera` y la propagación de `_frontera` por `a_mensual`/`rebasar`/`empalmar` (`conversion.py`). Estados: `cross_segmentado` (exacto), `cross_sin_frontera` (falta un ancla → el llamador conserva el visible de Fase 1, sin garantía). El alcance de clasificación estable NO se limita a componente/subcomponente: `_es_clasificacion_estable` también da `True` para `COG` y `CANASTA BASICA`, que reciben segmentación (algebraicamente exacta, sin indicador BIE mensual).
