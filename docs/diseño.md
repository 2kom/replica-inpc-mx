...
Fórmula: `inc_i = w_i × (J_i(t) − J_i(base)) / J_INPC(base)`, donde `J` es la escala **seleccionada por fila** (ver más abajo): `indice_incidencia` de-encadenado en filas within-canasta, `indice_replicado` visible en filas cross-canasta. La selección por fila es el contrato, no una excepción.
...
Las funciones de serie consumen la columna interna `indice_incidencia` (de-encadenada, vía `ResultadoIndice._completo`) y eligen la escala por fila `(periodo, indice)`: within-canasta usa `indice_incidencia` (exacto); cross-canasta de tipos con clasificación estable usa la descomposición exacta por segmentos, y de tipos finos cae al índice visible (sin garantía, `cross_visible`). El método por fila se marca en `metodo_incidencia` (`.reporte`/`.diagnostico`); el cruce es además detectable por `version_t != version_lag`. No se agrega ninguna columna nueva a `.resultado`. Detalle del contrato de de-encadenamiento en [5.11](#511-cálculo-de-variaciones-e-incidencias) y la decisión en [11.29](#1129-indice_incidencia-y-de-encadenamiento-de-incidencias).
...
**Cross-canasta exacto por segmentos.** Para todo tipo con clasificación estable (mismo conjunto de categorías y ningún genérico cruza de bucket en las 3 juntas) la fila cross se calcula exacta:
